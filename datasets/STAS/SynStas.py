...
os.makedirs('SynStas_Images',exist_ok=True)
os.makedirs('SynStas_Annotations',exist_ok=True)
...
for img_file, anno_file in zip(Images,Annotations):
    ...
    new_tree.write(f"{anno_file.replace('Train','SynStas')}", encoding='UTF-8')
    cv2.imwrite(f"{img_file.replace('Train','SynStas')}", new_img)
