...
for img_path in Train_Images:
    ...
    fw.write(f'{img_path},{np.median(hsv_img[:,:,0])},{np.median(hsv_img[:,:,1])},{np.median(hsv_img[:,:,2])}\n')
fw.close()
...
def cvt_img(img_paths, mean_A, mean_B):
    ...
    for img_path in img_paths:
        ...
        cv2.imwrite("ColorAdj_Images/"+str(img_path.split('\\')[-1]), new_img)

os.makedirs('ColorAdj_Images',exist_ok=True)
